...
@Component
public class NewContractListenerService2 {

	private static final Logger log = LoggerFactory.getLogger(NewContractListenerService2.class);
...
//						log.info("Approve transaction from: " + event.from + " to: "+ event.to + " ignored.");
				...
				// 忽略发送到marketAddress的approve及cancelList操作
				else if(Objects.equals(event.to, marketAddress2.toLowerCase()) || Objects.equals(event.from, marketAddress2.toLowerCase())){
//						log.info("Approve transaction from: " + event.from + " to: "+ event.to + " ignored.");
					return;
				}
...
//		Web3j web3j = Web3j.build(new HttpService(ETH_SEPOLIA_RPC, httpClient, false));
		...
		Credentials credentials = WalletUtils.loadCredentials(coinCreatePwd, coinKeystorePath + "/" + coinWalletFile);  // 2. 加载钱包
		BigInteger gasPrice = Contract.GAS_PRICE;
		Market2 contract = Market2.load(marketAddress2, web3j, credentials, gasPrice, gasLimit);  // 3. 加载合约
		...
		contract.itemPickedEventFlowable(startParameter, DefaultBlockParameterName.LATEST)  // 4. 订阅事件
			.subscribe(event -> {
				...
				if (txnHistoryCount > 0) {
					...
					txnHistoryPO.setToAddress(marketAddress2);
//					txnHistoryPO.setTime(event.log.getData());
					txnHistoryPO.initForUpdateNoAuth();
					txnHistoryMapper.insert(txnHistoryPO);
				}
		...
		//        typedResponse.shareAmount = (BigInteger) eventValues.getNonIndexedValues().get(5).getValue();
				...
				BigInteger sellerAmount = new BigInteger(String.valueOf(0));
				BigInteger shareAmount = new BigInteger(String.valueOf(0));
				...
				if (txnHistoryCount > 0) {
					...
					txnHistoryPO.setBlock(event.log.getBlockNumber().toString());
					txnHistoryPO.setFromAddress(marketAddress2);
...
//				}
		...
		//订阅refundPick事件
		contract.cancelPickItemEventFlowable(startParameter, DefaultBlockParameterName.LATEST)  // 4. 订阅事件
			.subscribe(event -> {

				// 这里处理事件
				log.info("refundPick pick event received: " + "buyer: " + event.buyer + " tokenId: " + event.tokenId + " pickIndex: " + event.index);
				String txnHash = event.log.getTransactionHash();
				// 查询txnHistory是否存在
				long txnHistoryCount = txnHistoryMapper.selectCount(new LambdaQueryWrapper<TxnHistoryPO>()
					.eq(TxnHistoryPO::getTxnHash, txnHash)
				...
				if (txnHistoryCount > 0) {
					...
					txnHistoryPO.setTokenId(event.tokenId.longValue());
					txnHistoryPO.setTxnHash(txnHash);
					txnHistoryPO.setMethod("refundPick");
					txnHistoryPO.setBlock(event.log.getBlockNumber().toString());
					txnHistoryPO.setFromAddress(marketAddress2);
					txnHistoryPO.setToAddress(event.buyer);
					txnHistoryPO.initForUpdateNoAuth();
				...
				// tokenid 位置 退款地址
				R result = marketService.refundPick(
					event.tokenId,
					event.index,
					event.buyer,
					event.log
